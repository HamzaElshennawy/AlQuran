...
class Reciter:
   ...
   def __toString__(self, withIndent: bool = False) -> str:
      ...
      """
      moshafList = f",\n{EDITOR_INDENT * 6 if withIndent else ""}".join(f"{str(moshaf) if withIndent else repr(moshaf)}" for moshaf in self.moshafList)
...
def toDataClass(data: dict, cls: type[T]) -> T:
   ...
   for field in fields(cls):
      ...
      # List of dataclasses
      elif hasattr(fieldType, "__origin__") and fieldType.__origin__ is list and is_dataclass(fieldType.__args__[0]):
