...
import com.hifnawy.alquran.shared.domain.CacheDataSource.cacheFactoryMap
import com.hifnawy.alquran.shared.domain.CacheDataSource.cacheInfo
import com.hifnawy.alquran.shared.domain.CacheDataSource.cacheLock
import com.hifnawy.alquran.shared.domain.CacheDataSource.cacheMap
...
object CacheDataSource {
    ...
    val CacheKey.cacheInfo
        get() = try {
            val cache = simpleCache
            val cachedSpans = cache.getCachedSpans(value)
            val totalBytes = cachedSpans.sumOf { it.length }
            val isCached = cachedSpans.isNotEmpty() && totalBytes > 0

            val cacheInfo = CacheInfo(cacheKey = this, isCached = isCached, cacheSize = totalBytes, cacheSizeHumanReadable = totalBytes.asHumanReadableSize)
            Timber.debug("$cacheInfo")

            cacheInfo
        } catch (ex: Exception) {
            Timber.error("Error checking cache: ${ex.message}")
            CacheInfo(this, false, -1, (-1L).asHumanReadableSize)
        }
